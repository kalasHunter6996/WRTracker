...
from gui.app_loader import ServicesLocator
...
class WRTrackerView(View):
    ...
    def _update(self):
        ...
        whole = int(wr) + 1
        self.as_setData('%.2f' % wr, '%.1f' % half, str(self._wins_to_target(wins, battles, half)), '%d|%d|%d' % (whole, self._wins_to_target(wins, battles, whole), battles))
    ...
    def as_setData(self, wr, half_target, half_wins, whole_data):
        print('[WRTracker] as_setData(%r, %r, %r, %r)' % (wr, half_target, half_wins, whole_data))
...
def setup():
    settings = ViewSettings(WR_TRACKER_VIEW, WRTrackerView, SWF_NAME, 0, None, ScopeTemplates.DEFAULT_SCOPE)
